    ...
    def process_pending_requests(self):
        ...
        while True:
            ...
            with Session(self.db_engine) as session:
                ...
                forward_requests_all = self.find_batchable_forward(session)
            ...
            min_forward_backward_id = (
                min(map(int, forward_backward_requests.keys())) if forward_backward_requests else None
            )
            forward_requests = (
                {
                    request_id: data
                    for request_id, data in forward_requests_all.items()
                    if min_forward_backward_id is None or int(request_id) < min_forward_backward_id
                }
                if forward_requests_all
                else {}
            )
