    ...
    if cfg.trainer.placement.colocate_all:
        if cfg.generator.backend != "vllm":
            raise NotImplementedError(
                "Colocated remote inference is currently supported only for the vLLM backend."
            )

        from skyrl_train.inference_engines.remote_inference_engine import launch_colocated_vllm_servers

        server_actor_handles, urls = launch_colocated_vllm_servers(cfg, colocate_pg)
